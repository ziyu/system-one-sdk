...
for (const [label, mutate] of [
  ...
  test(`invalid input is rejected before I/O: ${label}`, async () => {
    ...
    mutate(input);
    const client = new SystemOne({ adapter: systemOneAdapter, apiKey: () => assert.fail('key resolution must not run'), transport: createFetchTransport(async () => assert.fail('fetch must not run')) });
...
test('getters are rejected without executing them', async () => {
  ...
  const client = new SystemOne({ adapter: systemOneAdapter, apiKey: 'fixture', transport: createFetchTransport(async () => assert.fail('must not run')) });
...
for (const [label, mutate] of [
  ...
  test(`invalid provider response fails without a fallback: ${label}`, async () => {
    ...
    mutate(payload);
    let calls = 0;
    const client = new SystemOne({ adapter: systemOneAdapter, apiKey: 'fixture', transport: createFetchTransport(async () => { calls++; return jsonResponse(payload); }) });
...
test('provider-reported rounding is preserved rather than probabilities being silently normalized', async () => {
  ...
  const native = new SystemOne({ adapter: systemOneAdapter, apiKey: 'fixture', transport: createFetchTransport(async () => jsonResponse(payload)) });
  assert.equal((await native.evaluate(input)).answers.x.probabilities.a, 0.33);
  const gateway = new SystemOne({ adapter: vercelAdapter, apiKey: 'fixture', transport: createFetchTransport(async () => jsonResponse(payload)) });
...
test('missing distributions, confidence, and token counts stay absent for future providers', async () => {
  ...
  const client = new SystemOne({ adapter: vercelAdapter, apiKey: 'fixture', transport: createFetchTransport(async () => jsonResponse(payload)) });
...
test('unknown response fields are tolerated, while known fields are validated', async () => {
  ...
  const client = new SystemOne({ adapter: systemOneAdapter, apiKey: 'fixture', transport: createFetchTransport(async () => jsonResponse(payload)) });
...
test('future compatible models may support more options and levels than current Jev versions', async () => {
  ...
  const client = new SystemOne({ adapter: systemOneAdapter, baseURL: 'https://future.example/v1', model: 'future-1', apiKey: 'fixture', transport: createFetchTransport(async (_, init) => {
...
for (const baseURL of ['not a url', 'ftp://example.test', 'https://user:password@example.test', 'https://example.test?key=hidden', 'https://example.test#fragment']) {
  test('invalid or credential-bearing baseURL is rejected without echoing it', () => {
    assert.throws(() => new SystemOne({ adapter: systemOneAdapter, transport: createFetchTransport(), apiKey: 'fixture', baseURL }), error => error instanceof ConfigurationError && !error.message.includes('hidden') && !error.message.includes('password'));
  });
}

test('invalid client options and authentication fail as typed configuration errors', async () => {
  for (const options of [{ timeoutMs: 0 }, { maxRetries: -1 }, { retryDelayMs: Infinity }, { maxResponseBytes: 0 }, { model: 5 }]) {
    assert.throws(() => new SystemOne({ adapter: systemOneAdapter, transport: createFetchTransport(), apiKey: 'fixture', ...options }), ConfigurationError);
  }
  for (const apiKey of ['', undefined, ' leading', 'invalid\nkey', () => Promise.reject(new Error('hidden-key'))]) {
    const client = new SystemOne({ adapter: systemOneAdapter, apiKey, transport: createFetchTransport(async () => assert.fail('must not run')) });
