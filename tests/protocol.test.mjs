...
import { SystemOne, ConfigurationError, UnsupportedFeatureError, defineQuestions, choice, score, booleanQuestion } from '@system-one-ai/core';
...
for (const [baseURL, adapter, expected] of [
  ...
  test(`URL handling preserves the API prefix: ${baseURL}`, async () => {
    const client = new SystemOne({ baseURL, apiKey: 'fixture', adapter: adapter ?? systemOneAdapter, transport: createFetchTransport(async (url) => {
...
test('model IDs and provider options are explicit, without silently changing the requested model', async () => {
  ...
  const client = new SystemOne({ adapter: vercelAdapter, baseURL: 'https://ai-gateway.vercel.sh', apiKey: 'fixture', model: 'vendor/default', transport: createFetchTransport(async (_, init) => {
  ...
  const native = new SystemOne({ adapter: systemOneAdapter, apiKey: 'fixture', transport: createFetchTransport(async () => assert.fail('must fail before I/O')) });
...
test('the native client never selects a protocol or model based on the hostname', async () => {
  const client = new SystemOne({ adapter: systemOneAdapter, baseURL: 'https://ai-gateway.vercel.sh', apiKey: 'fixture', transport: createFetchTransport(async (url, init) => {
...
test('future models on the compatible protocol need only configuration changes', async () => {
  ...
  for (const [baseURL, model, apiKey] of [
    ...
    const client = new SystemOne({ adapter: systemOneAdapter, baseURL, model, apiKey, transport: createFetchTransport(async (url, init) => {
...
test('custom adapters can change request shape, response shape, and authentication', async () => {
  ...
  const client = new SystemOne({ baseURL: 'https://future.example/api', apiKey: 'future-fixture', adapter, transport: createFetchTransport(async (url, init) => {
  ...
  const unsupported = new SystemOne({ baseURL: 'https://future.example', apiKey: 'fixture', adapter: { ...adapter, supportedQuestionTypes: ['boolean'] }, transport: createFetchTransport(async () => assert.fail('must not run')) });
...
test('headers merge case-insensitively while credentials and protocol headers stay reserved', async () => {
  const client = new SystemOne({ adapter: systemOneAdapter, apiKey: 'fixture', headers: { 'X-Tenant': 'old' }, transport: createFetchTransport(async (_, init) => {
...
test('input is snapshotted before resolving an asynchronous key', async () => {
  ...
  const client = new SystemOne({ adapter: systemOneAdapter, apiKey: () => key, transport: createFetchTransport(async (_, init) => {
    const body = JSON.parse(init.body);
...
test('question builders preserve structured JSON instead of flattening it to a prompt', async () => {
  ...
  const client = new SystemOne({ adapter: systemOneAdapter, apiKey: 'fixture', transport: createFetchTransport(async (_, init) => {
...
test('keys such as __proto__ and constructor cannot pollute prototypes or disappear', async () => {
  ...
  const client = new SystemOne({ adapter: systemOneAdapter, apiKey: null, transport: createFetchTransport(async () => jsonResponse(payload)) });
