import { SystemOne, choice, type SystemOneAdapter } from '../../src/index.js';
import { cloudflareAdapter, type CloudflareAdapterOptions } from '../../src/adapters/cloudflare.js';
// @ts-expect-error Cloudflare is an optional entry point, never loaded by the core.
import { cloudflareAdapter as invalidRootExport } from '../../src/index.js';

const options: CloudflareAdapterOptions = { accountId: 'account-id' };
const adapter: SystemOneAdapter = cloudflareAdapter(options);
const client = new SystemOne({ adapter, apiKey: 'token' });
new SystemOne({ adapter, apiKey: 'token', baseURL: 'https://proxy.example/ai/run' });
...
new SystemOne({ adapter: cloudflareAdapter, apiKey: 'token' });
