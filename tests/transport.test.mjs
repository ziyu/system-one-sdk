...
import { SystemOne, APIError, TimeoutError, RequestAbortedError, ConnectionError, ResponseValidationError, ConfigurationError } from '../dist/esm/index.js';
import { parseRetryAfter } from '../dist/esm/transport.js';
...
for (const status of [408, 429, 500, 503, 529]) {
  test(`transient HTTP ${status} is retried with the same evaluation`, async () => {
    const bodies = [];
    const client = new SystemOne({ apiKey: 'fixture', retryDelayMs: 0, fetch: async (_, init) => {
...
for (const status of [400, 401, 403, 404, 409, 422]) {
  test(`HTTP ${status} retains status/request ID and is not retried`, async () => {
    let calls = 0;
    const client = new SystemOne({ apiKey: 'hidden-fixture-key', fetch: async () => {
      calls++;
...
test('retry exhaustion and per-request retry opt-out preserve the last error', async () => {
  let calls = 0;
  const client = new SystemOne({ apiKey: 'fixture', retryDelayMs: 0, fetch: async () => { calls++; return new Response('', { status: 529 }); } });
...
test('connection failures retry, without copying possibly sensitive fetch errors', async () => {
  let calls = 0;
  const client = new SystemOne({ apiKey: 'fixture', retryDelayMs: 0, fetch: async () => { calls++; throw new Error('echoed fixture key'); } });
...
test('server Retry-After is honored even when greater than the configured backoff cap', async () => {
  const times = [];
  const client = new SystemOne({ apiKey: 'fixture', maxRetryDelayMs: 0, fetch: async () => {
...
test('a Retry-After beyond the deadline never causes an early retry', async () => {
  let calls = 0;
  const client = new SystemOne({ apiKey: 'fixture', timeoutMs: 200, fetch: async () => { calls++; return new Response('', { status: 429, headers: { 'retry-after': '86400' } }); } });
...
test('an already-aborted call performs no authentication or network request', async () => {
  ...
  const client = new SystemOne({ apiKey: () => assert.fail('no key resolution'), fetch: async () => assert.fail('no fetch') });
...
test('timeout bounds a non-cooperative key resolver', { timeout: 2000 }, async () => {
  const client = new SystemOne({ apiKey: () => new Promise(() => {}), timeoutMs: 30, fetch: async () => assert.fail('no fetch') });
  await assert.rejects(client.evaluate(booleanRequest), TimeoutError);
});
...
test('timeout aborts and bounds a non-cooperative fetch', { timeout: 2000 }, async () => {
  let signal;
  let calls = 0;
  const client = new SystemOne({ apiKey: 'fixture', timeoutMs: 30, fetch: async (_, init) => { calls++; signal = init.signal; return new Promise(() => {}); } });
  await assert.rejects(client.evaluate(booleanRequest), TimeoutError);
...
test('timeout includes streamed body reading and cannot hang on cancellation', { timeout: 2000 }, async () => {
  ...
  const client = new SystemOne({ apiKey: 'fixture', timeoutMs: 30, fetch: async () => new Response(body) });
  await assert.rejects(client.evaluate(booleanRequest), TimeoutError);
...
test('caller cancellation interrupts Retry-After backoff and preserves cancellation semantics', { timeout: 2000 }, async () => {
  const controller = new AbortController();
  let calls = 0;
  const client = new SystemOne({ apiKey: 'fixture', fetch: async () => {
    calls++;
...
test('body-size enforcement preserves its error even when stream cancellation rejects', async () => {
  ...
  const client = new SystemOne({ apiKey: 'fixture', maxResponseBytes: 64, fetch: async () => new Response(body) });
...
test('content-length limits, malformed JSON, and invalid UTF-8 are rejected without retries', async () => {
  for (const response of [
    ...
    let calls = 0;
    const client = new SystemOne({ apiKey: 'fixture', maxResponseBytes: 1024, fetch: async () => { calls++; return response(); } });
...
test('cross-origin custom adapter requests fail before resolving credentials', async () => {
  ...
  const client = new SystemOne({ baseURL: 'https://expected.example', adapter, apiKey: () => assert.fail('key must not be resolved'), fetch: async () => assert.fail('no fetch') });
...
test('native fetch integration: real HTTP JSON request and redirect rejection', async t => {
  ...
  const client = new SystemOne({ baseURL: origin, apiKey: 'local-fixture' });
  const result = await client.evaluate(booleanRequest);
  assert.equal(result.answers.on.probability, 0.97);
  assert.equal(result.response.requestId, 'real-http-fixture');
  const redirecting = new SystemOne({ baseURL: `${origin}/redirect`, apiKey: 'local-fixture' });
